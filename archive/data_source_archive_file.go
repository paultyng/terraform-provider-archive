...
func archive(d *schema.ResourceData) error {
	...
	if dir, ok := d.GetOk("source_dir"); ok {
		...
		if err := arch.ArchiveContent([]byte(content), filename.(string)); err != nil {
		...
		if err := arch.ArchiveMultiple(content); err != nil {
			return fmt.Errorf("error archiving content: %s", err)
		}
	} else {
...
func genFileShas(filename string) (string, string, string, error) {
	...
	h := sha1.New()
	// this does not return an error, just matches io.Writer
	_, _ = h.Write(data)
